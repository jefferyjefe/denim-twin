...
if a.mask:
    ...
    hull = cv2.convexHull(np.array(list(lm.values()), np.float32)).astype(np.int32)
    gc = np.full(img.shape[:2], cv2.GC_PR_BGD, np.uint8); cv2.fillConvexPoly(gc, hull, cv2.GC_PR_FGD)
