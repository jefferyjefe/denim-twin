...
def check_image(path, board=None, spec=None, *, blur_min=80.0, clip_max=0.02,
                border_max=0.03, mean_range=(60, 200), min_corners=12):
    ...
    if board is not None:
        ...
        if r.board_corners < min_corners: r.fail(f"board corners {r.board_corners} < {min_corners}")
            ...
            pad = int(1.5 * spec['square_mm'] / r.mm_per_px) + 2 * int(10 / r.mm_per_px)  # outer squares + paper margin
    ...
    # exposure, excluding board
    g = gray[valid]
    r.mean_intensity = float(g.mean())
    r.clipped_fraction = float(((g <= 2) | (g >= 253)).mean())
    if not mean_range[0] <= r.mean_intensity <= mean_range[1]: r.fail("exposure out of range")
    if r.clipped_fraction > clip_max: r.fail(f"clipping {r.clipped_fraction:.1%}")
