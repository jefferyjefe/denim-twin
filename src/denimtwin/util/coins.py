...
def coin_key(text):
    t = (text or "").lower().replace("€2", "2 euro").replace("€1", "1 euro").replace("2€", "2 euro").replace("1€", "1 euro")
    if "cad" in t or "canad" in t: return "cad_quarter" if "quarter" in t else None
    if "aud" in t or "austral" in t: return "aud_1" if ("1" in t or "one" in t) else None
    if "50 cent" in t or "half dollar" in t: return "eur_50c" if ("euro" in t or "eur" in t) else None   # a US 50-cent piece is not in the table
