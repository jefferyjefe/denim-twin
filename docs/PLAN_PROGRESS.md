...
| §4.2 (tilt) | one upright implementation + estimator study + pair A/B | `canon/upright.py`, `tools/experiment_upright.py`, `tools/compare_upright_ab.py` | EXP_0022 deadband 8° → 0°; EXP_0023 read the near-vertical axis (the long axis is sideways on flat-laid shorts, so uprighting had been a no-op on 9 of 16 photos): tilt term in the repeatability suite 29.6% → 0.5%, pairs IoU 0.837 → 0.857, one pair regressing past the bench tolerance and left visible; EXP_0026 a waistband-edge estimator that is better measured and worse in the pipeline — not adopted |
| §4.3 Representation A (canonical 2D) | TPS canonical space | `canon/warp.py` | sub-pixel round-trip **at the landmarks only**: `CanonicalMap` fits two independent TPS maps, and over the garment the round-trip error is a median of 10.7 px and a worst case of 835 px; a *region* comes back with a median IoU of 0.638 with itself, worst 0.074 (EXP_0029). Every canonical quantity in the project inherits this |
...
| §2/§3 the product itself | one photo + cut spec -> prediction with interval, no ground truth needed | `tools/predict.py`, `tools/score_predict.py` | end-to-end tested; recomputed on the 7 pairs exclude.txt allows (EXP_0027): sil IoU **0.8026 product vs 0.8026 crop-only** — a dead heat — against 0.857 for the evaluation path that reads the after-photo |
