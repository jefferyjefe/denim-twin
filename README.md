...
complete session to READY and asserts the gate opens — a gate that cannot be opened by valid
evidence is broken, not safe. Most are there because an adversarial agent found them: independent
attackers in isolated worktrees produced 31 ways to make the gate say READY without the evidence,
and a second round aimed at the *fixes* produced 33 more — including that one of the round-1 fixes
was testing the record against itself, and that a hundred required frames were guarded by no gate at
all. Every one is now a scenario that fails without its fix. See
[`docs/PILOT_ADVERSARIAL_ROUNDS.md`](docs/PILOT_ADVERSARIAL_ROUNDS.md), which records what held as
well as what broke.
